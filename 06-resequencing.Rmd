...
pairedAFplot <- function(dat = dat, case = "Water", control = "Ciprofloxacin"){
	...
	data.wide <- dcast(dat.pair, genome + POS + REF + ALT + orf_desc ~ mouse.group, value.var = "AF")
    data.wide$vartype <- ifelse(as.character(data.wide$orf_desc) != "outside ORFs", "coding", "non-coding")
	...
	p <- ggplot(data.wide, aes(x = sample_1, y =sample_2, label = orf_desc, color = genome, shape =vartype))
...
data.wide <- dcast(dat, genome + POS + REF + ALT + orf_desc  ~ mouse.group, value.var = "AF")
data.wide <- data.wide[which(data.wide$genome == "KB1"),]
rownames(data.wide) <- paste0(data.wide$POS, " (",data.wide$REF, " -> ",data.wide$ALT, ") ", data.wide$orf_desc)
