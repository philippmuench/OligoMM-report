...
dat <- merge(dat, design, by = "mouse.id")
...
```{r}
dat$dummy <- 1
dat.by.sample.genome2 <- aggregate(dummy ~ mouse.id + genome + generation + ecoli + desc + day + DP, dat, sum)
colnames(dat.by.sample.genome2) <- c("sample", "genome", "generation", "ecoli", "desc","day","DP", "number_variants")
summary(dat$DP)
```
