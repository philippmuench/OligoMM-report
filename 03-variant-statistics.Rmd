...
dat[which(dat$orf_desc == "character(0)"),]$orf_desc <- NA
dat[which(dat$orf_desc == "hypothetical protein"),]$orf_desc <- NA
...
Filter by variants that change in AF from `< 0.5` to `>= 0.5` over time points.
...
pairedAFplot <- function(dat = dat, case = "1607", control = "1885"){
	...
	data.wide <- dcast(dat.pair, genome + POS + REF + ALT + orf_desc ~ mouse.id, value.var = "AF")
	colnames(data.wide)[6] <- "sample_1"
	colnames(data.wide)[7] <- "sample_2"
	data.wide[is.na(data.wide)] <- 0

	p <- ggplot(data.wide, aes(x = sample_1, y =sample_2, color = genome, label = orf_desc))
...
geneCentricPlot <- function(dat = dat, genome = "yl44"){
	require(reshape2) 
	require(ggplot2)
	dat.subset <- dat[which(dat$genome == genome & !is.na(dat$orf_desc)),]
	
	p <- ggplot(dat.subset, aes(x = orf_desc, y = AF, color = desc))
  p <- p + geom_point(size = 3, shape = "|", fill = NA)
	p <- p + coord_flip() + theme_classic()
	p <- p + ylim(c(0,1))
	p <- p + theme(strip.background = element_blank()) + theme(strip.text.y = element_text(angle = 180), strip.placement = "outside")
	 p <- p + xlab("") + ylab("allele frequency") 
	return(p)
}
```

 [1] "yl44" "yl27" "yl58" "i48"  "yl32" "i46"  "yl45" "yl31" "yl2"  "i49" 
[11] "kb18" "kb1" 

##### yl44

```{r, fig.align="center", fig.width=10, fig.height=10, fig.cap="AF by function of one genome"}
suppressWarnings(ggplotly(geneCentricPlot(dat, genome = "yl44")))
```

##### yl27

```{r, fig.align="center", fig.width=10, fig.height=10, fig.cap="AF by function of one genome"}
suppressWarnings(ggplotly(geneCentricPlot(dat, genome = "yl27")))
```

##### yl58

```{r, fig.align="center", fig.width=10, fig.height=10, fig.cap="AF by function of one genome"}
suppressWarnings(ggplotly(geneCentricPlot(dat, genome = "yl58")))
```

##### i48

```{r, fig.align="center", fig.width=10, fig.height=10, fig.cap="AF by function of one genome"}
suppressWarnings(ggplotly(geneCentricPlot(dat, genome = "i48")))
```

##### yl32

```{r, fig.align="center", fig.width=10, fig.height=10, fig.cap="AF by function of one genome"}
suppressWarnings(ggplotly(geneCentricPlot(dat, genome = "yl32")))
```

##### i46

```{r, fig.align="center", fig.width=10, fig.height=10, fig.cap="AF by function of one genome"}
suppressWarnings(ggplotly(geneCentricPlot(dat, genome = "i46")))
```

##### yl45

```{r, fig.align="center", fig.width=10, fig.height=10, fig.cap="AF by function of one genome"}
suppressWarnings(ggplotly(geneCentricPlot(dat, genome = "yl45")))
```

##### yl31

```{r, fig.align="center", fig.width=10, fig.height=10, fig.cap="AF by function of one genome"}
suppressWarnings(ggplotly(geneCentricPlot(dat, genome = "yl31")))
```

##### yl2

```{r, fig.align="center", fig.width=10, fig.height=10, fig.cap="AF by function of one genome"}
suppressWarnings(ggplotly(geneCentricPlot(dat, genome = "yl2")))
```

##### i49

```{r, fig.align="center", fig.width=10, fig.height=10, fig.cap="AF by function of one genome"}
suppressWarnings(ggplotly(geneCentricPlot(dat, genome = "i49")))
```
...
```{r, fig.align="center", fig.width=10, fig.height=10, fig.cap="AF by function of one genome"}
suppressWarnings(ggplotly(geneCentricPlot(dat, genome = "kb1")))
```
