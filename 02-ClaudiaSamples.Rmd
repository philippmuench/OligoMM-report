...
p <- ggplot(dat.agg, aes(x = reorder(mouse.id, -variants) , y = variants, color = mouse.group, shape = alteration_type))
p <- p + geom_jitter(shape = 4) + facet_grid(genome_hr ~., scales = "free_x")
