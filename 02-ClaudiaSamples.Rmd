...
data.wide$variant.id <- NULL
nrow(data.wide)
write.table(data.wide, file = "results/Claudia_samples_variants_wide.tsv", sep = "\t", row.names = F, quote=F)
heat <- data.wide
...
```{r, fig.cap="Position of variants. vertical lines show positions with functional annotation, if no annotation is shown, then its either hypothetical or outside ORF", fig.width=12, fig.height=8}
p <- createPositionPlot(chr = "Akkermansia_muciniphila")
p
```

```{r, fig.cap="Position of variants", fig.width=10, fig.height=6}
plotly::ggplotly(p)
```
