...
rule copy_files:
	...
	output:
		to1=temp("reads/pe/{sample}_R1_001.fastq.gz"),
		to2=temp("reads/pe/{sample}_R2_001.fastq.gz")
	shell: """
...
rule fastp_pe:
		...
		output:
				trimmed=[temp("trimmed/pe/{sample}.1.fastq.gz"), temp("trimmed/pe/{sample}.2.fastq.gz")],
